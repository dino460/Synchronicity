...
## How much energy drops while wandering per second.
var energy_decay : float


# This action shows how you can directly inject parameters.
func _init(p_wander_distance : float, p_energy_decay : float) -> void:
	self.wander_distance = p_wander_distance
	self.energy_decay = p_energy_decay
	print("WanderAction initialized with wander_distance: ", wander_distance)
...
func pre_perform_action(agent: GdPAIAgent) -> Action.Status:
	...
	var home_position = agent.blackboard.get_property("home_position")
	...
	if location_data.location_node_3d != null:
		...
		var wander_origin : Vector3 = home_position if home_position != null else location_data.position
		var target_location: Vector3 = wander_origin + random_dir_3d * wander_distance
...
func perform_action(
	...
	if nav_agent.is_navigation_finished() or (prior_positions.size() == 60 and dist_traveled < 0.001):
		return Action.Status.SUCCESS
