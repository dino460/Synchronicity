...
func get_npc_want(npc : NPC, is_at_home : bool, interference : float) -> float:
	...
	if time_can_leave > 23.999:
		...
		if ((scheduler.get_current_time() >= expected_home_time and scheduler.get_current_time() >= time_can_leave) or
			...
			stay_weight = (get_npc_reputation(npc.id) * npc.personality.aggression / npc.personality.loyalty)
		else:
			stay_weight = -scheduler.get_current_time() * npc.personality.loyalty / (time_want_to_arrive * get_npc_reputation(npc.id) * npc.personality.aggression)
...
func get_npc_attraction(npc_ref : NPC, is_current_location : bool, is_at_home : bool) -> float:
	...
	var lateness_weight = 0.0
	if not is_at_home:
		lateness_weight = lateness * npc_ref.personality.mind / npc_ref.personality.soul
	if lateness_weight < -1.0:
		lateness_weight = 0.0
	...
	if time_can_leave > 23.999:
		...
		if ((scheduler.get_current_time() >= expected_home_time and scheduler.get_current_time() >= time_can_leave) or
			(scheduler.get_current_time() <= expected_home_time and scheduler.get_current_time() <= time_can_leave)):
			stay_weight = (get_npc_reputation(npc_ref.id) * npc_ref.personality.aggression / npc_ref.personality.loyalty)
		else:
			stay_weight = -scheduler.get_current_time() * npc_ref.personality.loyalty / (time_want_to_arrive * get_npc_reputation(npc_ref.id) * npc_ref.personality.aggression)
	elif scheduler.get_current_time() >= expected_home_time and scheduler.get_current_time() <= time_can_leave:
		stay_weight = (get_npc_reputation(npc_ref.id) * npc_ref.personality.aggression / npc_ref.personality.loyalty)
	else:
		stay_weight = scheduler.get_current_time() * npc_ref.personality.loyalty / (time_want_to_arrive * get_npc_reputation(npc_ref.id) * npc_ref.personality.aggression)
	...
	return super(npc_ref, is_current_location, is_at_home) + lateness_weight + stay_weight
